...
draw pnl inp dc rt = do
    ...
    --print res
    mapM_ (drect dc) res
    where drect dc (Block x y bl)  = drawRect dc (rect (pt x y) (sz 95 50)) (if bl then [brush := BrushStyle BrushSolid yellow] else [])
          drect dc (Name str x y)  = drawText dc str (pt x y) []
          drect dc (Line (x1,y1) (x2,y2))
                                   = line dc (pt x1 y1) (pt x2 y2) []
          drect dc (WTam w h)      = do sw@(Size tw th) <- get pnl size
